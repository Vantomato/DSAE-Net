...
parser.add_argument('--path_train_preds', type=str, default='results/DRIVE/experiments/wnet_drive', help='path to training predictions')
parser.add_argument('--path_test_preds', type=str, default='results/DRIVE/experiments/wnet_drive', help='path to test predictions')
...
def get_labels_preds(path_to_preds, csv_path):
    ...
    for i in range(len(gt_list)):
        ...
        im_path = os.path.splitext(os.path.basename(im_path))[0]
        pred_path = osp.join(path_to_preds, im_path[:-4] + '.png')
        pred_path = osp.join(path_to_preds, im_path + '.png')
        gt_path = gt_list[i]
    ...
    return np.hstack(all_preds), np.hstack(all_gts), all_preds, all_gts
...
def compute_performance(preds, gts, save_path=None, opt_threshold=None, cut_off='dice', mode='train',all_preds=None, all_gts=None):
    ...
    mccs = []
    if all_preds is not None and all_gts is not None:
        # 计算每个样本的mcc
        mccs = []
        for i in range(len(all_gts)):
            gts = np.hstack(all_gts[i])
            preds = np.hstack(all_preds[i])
            bin_preds = preds > opt_threshold
            mccs.append(matthews_corrcoef(gts.astype(int), bin_preds.astype(int)))

    return global_auc, acc, dice, mcc, specificity, sensitivity, opt_threshold, mccs

def analyze_results_one(config):
    train_dataset = config.train_dataset
    test_dataset = config.test_dataset
    path_train_preds = config.path_train_preds
    path_test_preds = config.path_test_preds
    cut_off = config.cut_off
    public = str2bool(config.public)
    ...
    preds, gts, all_preds, all_gts = get_labels_preds(path_train_preds, csv_path = csv_path)
    os.makedirs(save_path, exist_ok=True)
    global_auc_tr, acc_tr, dice_tr, mcc_tr, spec_tr, sens_tr, opt_thresh_tr,_ =\
    ...
    preds, gts, all_preds, all_gts = get_labels_preds(path_train_preds, csv_path = csv_path)
    global_auc_vl, acc_vl, dice_vl, mcc_vl, spec_vl, sens_vl, _,_ =\
    ...
    preds, gts, all_preds, all_gts = get_labels_preds(path_test_preds, csv_path = path_test_csv)
    global_auc_test, acc_test, dice_test, mcc_test, spec_test, sens_test, _,mccs = \
        compute_performance(preds, gts, save_path=save_path, opt_threshold=opt_thresh_tr,all_preds=all_preds, all_gts=all_gts)
    perf_df_test = pd.DataFrame({'auc': global_auc_test,
                                 ...
                                 'spec': spec_test,
                                 'sens': sens_test,
                                 'mccs': str(mccs),}, index=[0])
    perf_df_test.to_csv(perf_csv_path, index=False)

def analyze_results_all():
    config = argparse.Namespace()
    model_name_arr = ['My76_WNet', 'UNettp', 'AttU_Net', 'SA_UNet', 'ConvUNeXt', 'ULite']
    dataset_arr = ['DRIVE', 'STARE', 'CHASEDB1', 'HRF']
    for model_name in model_name_arr:
        for dataset in dataset_arr:
            config.model_name = model_name
            config.dataset = dataset
            config.path_train_preds = f'results/{dataset}/experiments/{model_name}_{dataset}'
            config.path_test_preds = f'results/{dataset}/experiments/{model_name}_{dataset}'
            config.train_dataset = f'{dataset}'
            config.test_dataset = f'{dataset}'
            config.cut_off = 'dice'
            config.public = True
            analyze_results_one(config)
