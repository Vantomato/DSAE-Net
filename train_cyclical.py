...
def train_model(model, optimizer, criterion, train_loader, val_loader, scheduler, grad_acc_steps, metric, exp_path):
    ...
    if os.path.exists(osp.join(exp_path, 'train_logs.csv')):
        os.remove(osp.join(exp_path, 'train_logs.csv'))
    for cycle in range(n_cycles):
        ...
        if exp_path is not None:
            train_logs = osp.join(exp_path, 'train_logs.csv')
            if not osp.exists(train_logs):
                with open(train_logs, 'w') as f:
                    print('cycle, t_loss, t_auc, t_dice, v_loss, v_auc, v_dice', file=f)
            with open(train_logs, 'a') as f:
                print('{},{},{},{},{},{},{}'.format(cycle+1, tr_loss, tr_auc, tr_dice, vl_loss, vl_auc, vl_dice), file=f)
        print('Monitoring metric: {} = {:.4f}'.format(metric, monitoring_metric))
