...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index >= len(self.im_list):
            index = index % len(self.im_list)
...
def get_train_val_datasets(csv_path_train, csv_path_val, tg_size=(512, 512), label_values=(0, 255)):
    ...
    rotate1 = p_tr.RandomChoice([
        p_tr.RandomRotation(degrees=[0, 0], fill=(0, 0, 0), fill_tg=(0,)),
        p_tr.RandomRotation(degrees=[90, 90], fill=(0, 0, 0), fill_tg=(0,)),
        p_tr.RandomRotation(degrees=[180, 180], fill=(0, 0, 0), fill_tg=(0,)),
        p_tr.RandomRotation(degrees=[270, 270], fill=(0, 0, 0), fill_tg=(0,))
    ])
    ...
    val_transforms = p_tr.Compose([resize, rotate1, h_flip, v_flip, tensorizer])
...
if __name__ == '__main__':
    path = r'D:\\study\\experiment\\lwnet-master\/data/DRIVE/train.csv'
    train_dataset = TrainDataset(csv_path=path)
    train_dataset[0]
    pass